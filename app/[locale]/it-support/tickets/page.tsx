...
function ResolveModal({ ticket, onClose, onConfirm }: ResolveModalProps) {
            ...
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
                ...
                <div className="px-5 py-4 space-y-3">
                    ...
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-1">
                            Ghi chú xử lý <span className="text-slate-400 font-normal">(tuỳ chọn)</span>
                        </label>
                        <textarea
                            value={note}
                            onChange={e => setNote(e.target.value)}
                            placeholder="Mô tả cách đã xử lý vấn đề kỹ thuật..."
                            rows={3}
                            className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 placeholder-slate-300 resize-none focus:outline-none focus:ring-2 focus:ring-purple-300 focus:border-purple-400"
                        />
                    </div>
                </div>
                <div className="flex gap-2 px-5 pb-5">
                    ...
                    <button
                        ...
                        disabled={loading}
                        className="flex-1 px-4 py-2 text-sm font-semibold text-white bg-purple-600 rounded-lg hover:bg-purple-700 transition-colors disabled:opacity-60 flex items-center justify-center gap-2"
                    >
    ...
    const handleResolve = async (note: string) => {
        ...
        try {
            ...
            setTickets(prev => prev.map(t =>
                t.id === resolvingTicket.id ? { ...t, status: "SOLVED" as any, resolveNote: note } : t
            ));
        } catch {
        ...
        <div className="min-h-full p-4 md:p-6">
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
                        ...
                        Ticket kỹ thuật của tôi
                    </h1>
                    <p className="text-sm text-slate-500 mt-0.5">Các sự cố kỹ thuật được giao để bạn xử lý</p>
                </div>
                <button
                ...
                {FILTER_TABS.map(tab => (
                    ...
                    >
                        ...
                        <span className={cn(
                            ...
                            filter === tab.key ? "bg-purple-400 text-white" : "bg-slate-100 text-slate-500"
                        )}>
                            {counts[tab.key] ?? 0}
                        </span>
                    </button>
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 gap-3 text-slate-400">
                    ...
                    <p className="text-xs text-slate-300">Khảo thí chưa giao ticket kỹ thuật cho bạn</p>
                </div>
            ) : (
                        ...
                        return (
                            <div
                                ...
                                className={cn(
                                    ...
                                    ticket.status === "IN_PROGRESS" ? "border-purple-200" : "border-slate-200"
                                )}
                            >
                                {/* Title + badges */}
                                ...
                                <div className="flex items-center gap-3 mt-1.5 text-xs text-slate-500 flex-wrap">
                                    {ticket.reporter?.fullName && (
                                        <span className="font-semibold text-slate-600">
                                            Báo cáo bởi: {ticket.reporter.fullName}
                                        </span>
                                    )}
                                    {roomNumber && (
                                ...
                                {canResolve && (
                                    <div className="mt-3 pt-3 border-t border-slate-100">
                                        <button
                                            ...
                                            className="flex items-center gap-2 px-4 py-1.5 text-sm font-semibold text-white bg-purple-600 rounded-lg hover:bg-purple-700 transition-colors"
                                        >
