...
export default function SeatingPlan({
    ...
    const startSwapFromSeat = (seat: ExamSeat) => {
        ...
        toast.info(t("swapModeEnabled", { seat: `R${seat.row}C${seat.col}` }));
    };
    ...
    const handleSwapTargetSelect = async (targetSeat: ExamSeat) => {
        ...
        if (targetSeat.id === swapSourceSeat.id) {
            setActionError(t("swapTargetError"));
            return;
        }
        ...
        try {
            ...
            if (!result.success) {
                setActionError(result.error || t("swapError"));
                return;
            }

            toast.success(t("swapSuccess", { source: `R${swapSourceSeat.row}C${swapSourceSeat.col}`, target: `R${targetSeat.row}C${targetSeat.col}` }));
            setSwapSourceSeat(null);
            ...
            {swapSourceSeat && (
                <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg flex items-center justify-between gap-3">
                    <p className="text-[12px] text-blue-700 font-medium">
                        {t("swapModeHint", { seat: `R${swapSourceSeat.row}C${swapSourceSeat.col}` })}
                    </p>
                    <button
                        type="button"
