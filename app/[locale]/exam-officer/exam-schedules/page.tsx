...
export default function ExamOfficerDashboardPage() {
  ...
  const handleExport = async (selectedCampus: string) => {
    try {
      const exportParams = { ...searchParams };
      if (selectedCampus) {
          exportParams.campus = selectedCampus;
      }
      
      const blob = await examSchedulesApi.export(exportParams);
      const url = window.URL.createObjectURL(new Blob([blob]));
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", `StudentReport_${selectedCampus || "All"}_${format(new Date(), "dd-MM-yyyy")}.xlsx`);
  ...
  return (
    <div className="flex flex-col gap-6 p-4 md:p-6 bg-[#f0f2f5] min-h-screen">
      ...
      <ExportReportDialog isOpen={showExportDialog} onClose={() => setShowExportDialog(false)} onExport={handleExport} />
        ...
        <div className="fixed inset-0 z-[9998] flex justify-end">
          ...
          <div className="relative w-[480px] h-full bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            <div className="p-5 bg-slate-900 text-white flex items-center justify-between shrink-0">
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-white/50 mb-1">All Rooms</p>
                <h3 className="text-lg font-black">{expandedSessions.label}</h3>
                <p className="text-xs text-white/60 font-bold mt-0.5">{expandedSessions.sessions.length} sessions total</p>
              </div>
              ...
              {expandedSessions.sessions.map((s: any) => (
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-3 text-xs font-bold text-slate-500">
                    <span className="flex items-center gap-1.5 text-blue-700 bg-blue-50 px-2 py-1 rounded-lg border border-blue-100"><MapPin className="h-3 w-3" /> {s.roomNumber || '---'}</span>
